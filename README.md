...
hand.  
  ...
  During equity calculation, the solver continuously checks if a hand’s EV is already above or below a certain threshold.  
  If the result is decisive, further simulation for that hand stops early.  

  This significantly reduces solving time, but may also introduce small anomalies:  
  some hands can be misclassified if they converge too early.  
  For that reason, results should always be compared against *similar hands* to verify correctness.
